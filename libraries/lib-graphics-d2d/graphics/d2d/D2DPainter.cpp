...
void D2DPainter::UpdateFont(std::shared_ptr<PainterFont> font)
{
   if (font != nullptr && font->GetRendererID() == GetRendererID())
      mCurrentD2DFont = std::static_pointer_cast<D2DFont>(font);
   else
   ...
   const std::string_view& text)
{
   ...
   mCurrentD2DFont->Draw(*mRenderTargetStack.back(), origin, backgroundBrush, text);
}
   ...
   const std::string_view& text)
{
   ...
   mCurrentD2DFont->Draw(*renderTarget, Point {}, backgroundBrush, text);

   d2dRenderTarget->SetTransform(currentTransform);
}

Size D2DPainter::DoGetTextSize(const std::string_view& text, bool gridFitted) const
{
