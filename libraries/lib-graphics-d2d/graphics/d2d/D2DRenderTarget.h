...
//! A base class that wraps around the Direct2D render target.
/*!
 * Direct2D has a complex notion of the resource ownership.
 * Unlike Direct3D or OpenGL, most of the Direct2D resource are specific to the render target,
 * not the rendering device. In some cases ID2D1Bitmap objects can be shared, but implementation
 * tries not to rely on this. Instead, it `acquires` a new copy of the RT dependent resources for
 * each render target and releases them when the RT is destroyed.
 *
 * The only case where resources are considered compatible is when the resource is requested from
 * the nested paint event. In this case, the resource is acquired from the parent render target,
 * which reduces the memory overhead.
 *
 * On top of that brushes and pens are cached, which are surprisingly considered to be render target
 * resources as well.
 *
 */
