...
//! Font collection to cache D2DFonts
class D2DFontCollection final
{
public:
   D2DFontCollection(const RendererID& rendererId, IDWriteFactory* factory);
...
private:
   using FontCollection =
      std::map<std::pair<uint32_t, FontInfo>, std::shared_ptr<D2DFont>>;
