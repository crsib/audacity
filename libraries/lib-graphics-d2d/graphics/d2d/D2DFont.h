...
//! DirectWrite based PainterFont implementation
/*!
 * DirectWrite uses quite unusual naming:
 *   * IDWriteTextFormat encapsulates the font to be used
 *   * IDWriteTextLayout stores the shaped text to be drawn
 *
 * IDWriteTextLayout is expensive to create, so we cache it in an LRU cache.
 * Given that Audacity paints only a relatively small amount of strings using
 * the painter - this provides a reasonable performance at a low memory overhead.
 *
 * Given that Audacity is configured as a DPI-unaware application at this point,
 * DPI is always considered to be 96.
 */
