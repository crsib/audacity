...
void D2DRenderTargetBitmap::DrawFinished(D2DRenderTarget& renderTarget)
{
   auto it = mRTDependentData.find(&renderTarget);

   mLastPaintedBitmap =
      it != mRTDependentData.end() ? it->second.Bitmap : nullptr;

   mLastPaintedRenderTarget = renderTarget.GetD2DRenderTarget();
}
...
std::vector<uint8_t> D2DRenderTargetBitmap::GetData() const
{
   if (mLastPaintedBitmap)
      return {};
   
   return GetRenderer().GetImageData(mLastPaintedRenderTarget, mLastPaintedBitmap.Get(), mHasAlpha);
}

bool D2DRenderTargetBitmap::HasAlpha() const noexcept
{
   return mHasAlpha;
}
