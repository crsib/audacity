...
{
   DoDrawText(origin, Brush::NoBrush, text);
}

void Painter::DrawText(float x, float y, const std::string_view& text)
{
   DoDrawText(Point { x, y }, Brush::NoBrush, text);
}
...
{
   DrawText(
      Rect { origin, size }, text, horizontalAlignement, verticalAlignment);
}
...
{
   DrawText(
      Rect { Point { x, y }, Size { w, h } }, text, horizontalAlignement,
      verticalAlignment);
}
...
{
   DoDrawRotatedText(origin, angle, Brush::NoBrush, text);
}
...
void PainterOffscreenHolder::Flush()
{
   if (mSurface != nullptr)
   {
...
bool Painter::IsRectClipped(float x, float y, float width, float height) const noexcept
{
   return IsRectClipped(Rect { Point { x, y }, Size { width, height } });
}

bool Painter::IsRectClipped(Point point, Size size) const noexcept
{
