...
void GLPainter::UpdateFont(std::shared_ptr<PainterFont> font)
{
   if (font != nullptr && font->GetRendererID() == GetRendererIndependentID())
      mCurrentFont = std::static_pointer_cast<fonts::Font>(font);
   else
      UpdateFont(GetDefaultFont());
}
   ...
   const std::string_view& text)
{
   ...
   DoDrawRotatedText(origin, 0.0f, backgroundBrush, text);
}
   ...
   const std::string_view& text)
{
   ...
   auto size = mCurrentFont->GetTextSize(text, !hasRotation);
   ...
   mCurrentPaintTarget->SetDefaultShader();
   mCurrentFont->DrawText(mRenderer.GetFontRenderer(), text, mCurrentBrush.GetColor());

   mCurrentPaintTarget->SetTransform(mCurrentTransform);
}

Size GLPainter::DoGetTextSize(const std::string_view& text, bool gridFitted) const
{
