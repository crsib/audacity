...
public:
   ...
   std::vector<uint8_t> GetData(const RectType<uint32_t>& rect) const
   {
      auto& context = mRenderer.GetResourceContext();
      auto& functions = context.GetFunctions();
      ...
      std::vector<uint8_t> data(bytesPerPixel * mWidth * mHeight);
      ...
      functions.GetTexImage(
         GLenum::TEXTURE_2D, 0,
         mFormat == PainterImageFormat::RGB888 ? GLenum::RGB : GLenum::RGBA,
         GLenum::UNSIGNED_BYTE, data.data());

      if (currrentTexture != nullptr)
         functions.BindTexture(GLenum::TEXTURE_2D, currrentTexture->mSharedGLObject->mTextureID);

      for (uint32_t row = 0; row < mHeight / 2; ++row)
      {
         const auto row1 = row * mWidth * bytesPerPixel;
         const auto row2 = (mHeight - row - 1) * mWidth * bytesPerPixel;

         for (uint32_t subPixel = 0; subPixel < mWidth * bytesPerPixel; ++subPixel)
            std::swap(data[row1 + subPixel], data[row2 + subPixel]);
      }

      if (
         rect.origin.x == 0 && rect.origin.y == 0 &&
         rect.size.width == mWidth && rect.size.height == mHeight)
         return data;

      for (uint32_t row = 0; row < mHeight; ++row)
      {
         uint8_t* outPtr = row * mWidth * bytesPerPixel + data.data();

         const uint8_t* inPtr = (row + rect.origin.y) * mWidth * bytesPerPixel +
                                rect.origin.x * bytesPerPixel + data.data();

         std::memmove(outPtr, inPtr, rect.size.width * bytesPerPixel);
      }

      data.resize(rect.size.width * rect.size.height * bytesPerPixel);
...
std::vector<uint8_t> Texture::GetData() const
{
   return mSharedGLObject->GetData(mTextureRect);
}
