...
// setContentsChanged is not present in the CALayer interface,
// but is wildly used by browsers anyway.
@interface CALayer (Private)
- (void)setContentsChanged;
@end

// NSView subclass, that uses IOSurface to draw the content.
...
#else
   // This branch can be helpfull for debugging.
   // macOS OpenGL profiler is not able to handle IOSurface backed textures.
...
ViewSurface::ViewSurface(const GLFunctions& functions, void* view)
   : mImpl(
      std::make_unique<Impl>(functions, static_cast<NSView*>(view)))
{
}
