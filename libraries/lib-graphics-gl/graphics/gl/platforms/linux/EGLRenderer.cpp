...
// This code is disabled for now, because it is not clear how EGL_HORIZONTAL_RESOLUTION is
// really implemented.
#if 0
      if(mSurface != nullptr)
      {
         ...
         if (pixelsPerInch == 0)
            ...
            // EGL uses pixels-per-cm, instead of pixels-per-inch.
...
bool EGLRenderer::IsAvailable() const noexcept
{
...
std::unique_ptr<Context> EGLRenderer::CreateContext(WindowHandle window)
{
