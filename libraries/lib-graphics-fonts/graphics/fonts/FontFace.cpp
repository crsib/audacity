...
// RAII wrapper for the FT_Library
...
// FT_Library is a singleton
FreeTypeLibrary libraryInstance;

// RAII wrapper for the hb_font_t
// This class is used to shape the text
...
public:
       ...
       , mPixelSize(pixelSize)
   {
      ...
      // FT_Set_Pixel_Sizes must be called first
   ...
   std::shared_ptr<TextLayout> CreateTextLayout(std::string_view text, bool hinted) const
   {
      ...
      // FT_Set_Pixel_Sizes is called again, because FreeType will be used to calculate
      ...
      {
         const auto metrics =
            GetCachedMetrics(info[symbolIndex].codepoint, hinted);

         symbols.emplace_back(TextLayoutSymbol { info[symbolIndex].codepoint,
                                                 info[symbolIndex].cluster,
                                                 currentX, currentY });
         ...
         // Calculate the kerning if needed
         ...
         if (FT_HAS_KERNING(mFreeTypeFace))
         {
            FT_Get_Kerning(
               ...
               info[symbolIndex + 1].codepoint, FT_KERNING_DEFAULT, &kerning);
         }

         const auto secondSymbolMetrics =
            GetCachedMetrics(info[symbolIndex + 1].codepoint, hinted);

         // FreeType 2 computes the "errors" when performing the hinting
         // This information is used to improve the glyph positioning
...
private:
   ...
   GlyphMetrics ComputeMetrics(hb_codepoint_t codePoint, bool hinted) const noexcept
   {
      ...
      return { { int(mFreeTypeFace->glyph->advance.x / 64),
                 int(mFreeTypeFace->glyph->advance.y / 64) },
...
// Private implementation of the FontFace
...
public:
       ...
       : mFontStream(std::move(fontStream))
   {
      ...
      // As per FreeType docs, 0x7FFFFFFF can be used
      // if we do not want to provide the stream size
      constexpr auto UnknownSize = 0x7FFFFFFF;
      mFreeTypeStream.size = UnknownSize;
      ...
      // Error checking is not performed here, because
      // later mFreeTypeFace will be checked for nullptr
      FT_Open_Face(
         libraryInstance.library, &mFreeTypeOpenArgs, 0, &mFreeTypeFace);
   }
